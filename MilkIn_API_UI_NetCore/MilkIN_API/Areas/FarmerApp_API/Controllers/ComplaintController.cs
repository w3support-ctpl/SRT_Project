...
namespace MilkIN_API.Areas.FarmerApp_API.Controllers
{
    ...
    public class ComplaintController : Controller
    {
        ...
        public ComplaintController(ILogger<ComplaintController> logger, IConfiguration configuration)
        {
            _logger = logger;
            _configuration = configuration;
        }

        [HttpPost("GetComplaint", Name = "GetComplaint")]
        public IActionResult GetComplaint([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_FarmerComplaint_Get");
        ...
        [HttpPost("SaveComplaint", Name = "SaveComplaint")]
        public IActionResult SaveComplaint([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_FarmerComplaint_Set");
