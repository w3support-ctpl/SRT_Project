...
{
    [Route("v1/api/driver/triphistory/")]
    [ApiController]
    public class TripHistoryController : Controller
    {
        private readonly ILogger<TripHistoryController> _logger;

        private readonly IConfiguration _configuration;
        public TripHistoryController(ILogger<TripHistoryController> logger, IConfiguration configuration)
        {
            _logger = logger;
            _configuration = configuration;
        }

        [HttpPost("GetTripHistory", Name = "GetTripHistory")]
        public IActionResult GetTripHistory([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_DriverTripHistory_Get");
