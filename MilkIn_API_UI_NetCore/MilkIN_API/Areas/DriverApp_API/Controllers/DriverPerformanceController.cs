...
{
    [Route("v1/api/driver/performance/")]
    [ApiController]
    public class DriverPerformanceController : Controller
    {
        ...
        public DriverPerformanceController(ILogger<DriverPerformanceController> logger, IConfiguration configuration)
        {
            _logger = logger;
            _configuration = configuration;
        }

        [HttpPost("DriverGetPerformance", Name = "DriverGetPerformance")]
        public IActionResult DriverGetPerformance([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_DriverPerformance");
