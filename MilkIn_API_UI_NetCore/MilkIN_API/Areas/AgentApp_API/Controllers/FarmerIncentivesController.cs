...
namespace MilkIN_API.Areas.AgentApp_API.Controllers
{
    ...
    [ApiController]
    public class FarmerIncentivesController : Controller
    {
        private readonly ILogger<FarmerIncentivesController> _logger;

        private readonly IConfiguration _configuration;

        public FarmerIncentivesController(ILogger<FarmerIncentivesController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("GetFarmerIncentives", Name = "GetFarmerIncentives")]
        public IActionResult GetFarmerIncentives([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentFarmerIncentiveScheme_Get");
        ...
        [HttpPost("SaveFarmerIncentives", Name = "SaveFarmerIncentives")]
        public IActionResult SaveFarmerIncentives([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentFarmerIncentiveScheme_Set");
