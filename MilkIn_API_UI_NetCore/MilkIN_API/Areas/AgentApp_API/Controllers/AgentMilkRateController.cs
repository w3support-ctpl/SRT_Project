...
{
    [Route("v1/api/agent/milkrate/")]
    [ApiController]
    public class AgentMilkRateController : Controller
    {
        private readonly ILogger<AgentMilkRateController> _logger;

        private readonly IConfiguration _configuration;

        public AgentMilkRateController(ILogger<AgentMilkRateController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("GetMilkRateOffline", Name = "GetMilkRateOffline")]
        public IActionResult GetMilkRateOffline([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentMilkRate_Get");
        ...
        [HttpPost("SaveMilkRateOffline", Name = "SaveMilkRateOffline")]
        public IActionResult SaveMilkRateOffline([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentMilkRate_Set");
