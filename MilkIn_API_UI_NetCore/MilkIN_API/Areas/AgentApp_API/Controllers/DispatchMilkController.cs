...
{
    [Route("v1/api/agent/dispatch/")]
    [ApiController]
    public class DispatchMilkController : Controller
    {
        private readonly ILogger<DispatchMilkController> _logger;

        private readonly IConfiguration _configuration;

        public DispatchMilkController(ILogger<DispatchMilkController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("GetVehicleTripStatus", Name = "GetVehicleTripStatus")]
        public IActionResult GetVehicleTripStatus([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentGetVehicle_Status");
        ...
        [HttpPost("DispatchMilk", Name = "DispatchMilk")]
        public IActionResult DispatchMilk([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentMilkDispatch");
