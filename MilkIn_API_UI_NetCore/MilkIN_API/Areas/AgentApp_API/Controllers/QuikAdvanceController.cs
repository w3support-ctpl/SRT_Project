...
namespace MilkIN_API.Areas.AgentApp_API.Controllers
{
    ...
    [ApiController]
    public class QuikAdvanceController : Controller
    {
        private readonly ILogger<QuikAdvanceController> _logger;

        private readonly IConfiguration _configuration;

        public QuikAdvanceController(ILogger<QuikAdvanceController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("GetQuickAdvance", Name = "GetQuickAdvance")]
        public IActionResult GetQuickAdvance([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentAdvance_Get");
        ...
        [HttpPost("ApproveAdvance", Name = "ApproveAdvance")]
        public IActionResult ApproveAdvance([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentApproveAdvance_Set");
