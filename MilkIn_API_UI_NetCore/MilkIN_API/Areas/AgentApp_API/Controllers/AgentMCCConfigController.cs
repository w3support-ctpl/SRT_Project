...
{
    [Route("v1/api/agent/config/")]
    [ApiController]
    public class AgentMCCConfigController : Controller
    {
        private readonly ILogger<AgentMCCConfigController> _logger;

        private readonly IConfiguration _configuration;

        public AgentMCCConfigController(ILogger<AgentMCCConfigController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("GetAgConfig", Name = "GetAgConfig")]
        public IActionResult GetAgConfig([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentMCCConfig_Get");
        ...
        [HttpPost("SaveAgConfig", Name = "SaveAgConfig")]
        public IActionResult SaveAgConfig([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentMCCConfig_Set");
