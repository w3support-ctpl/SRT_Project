...
{
    [Route("v1/api/agent/stock/")]
    [ApiController]
    public class ManageStockController : Controller
    {
        private readonly ILogger<ManageStockController> _logger;

        private readonly IConfiguration _configuration;

        public ManageStockController(ILogger<ManageStockController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("ManageStocks", Name = "ManageStocks")]
        public IActionResult ManageStocks([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentManageStock");
        ...
        [HttpPost("StocksDispatch", Name = "StocksDispatch")]
        public IActionResult StocksDispatch([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentStocks_Dispatch");
