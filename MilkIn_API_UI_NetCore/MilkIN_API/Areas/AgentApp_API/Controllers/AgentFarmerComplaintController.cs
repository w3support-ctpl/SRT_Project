...
namespace MilkIN_API.Areas.AgentApp_API.Controllers
{
    ...
    [ApiController]
    public class AgentFarmerComplaintController : Controller
    {
        private readonly ILogger<AgentFarmerComplaintController> _logger;

        private readonly IConfiguration _configuration;

        public AgentFarmerComplaintController(ILogger<AgentFarmerComplaintController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [HttpPost("AgFarmerGetComplaint", Name = "AgFarmerGetComplaint")]
        public IActionResult AgFarmerGetComplaint([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentFarmerComplaint_Get");
        ...
        [HttpPost("AgFarmerSaveComplaint", Name = "AgFarmerSaveComplaint")]
        public IActionResult AgFarmerSaveComplaint([FromBody] object reqObject)
        {
            try
            {
                if (reqObject == null)
                ...
                dynamic inputParam = JsonConvert.DeserializeObject(reqObject.ToString());
                string destination_name = inputParam.Destination_Name;
                string res_Str = new CommonDAL(destination_name, _configuration).RunDBQuery(inputParam, "USP_AgentFarmerComplaints_Set");
