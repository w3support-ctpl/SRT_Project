...
namespace MilkOUT_API.Areas.AdminConsole_API.SAP
{
    public class CreditMemoSAP
    {
        ...
        public CreditMemoSAP(string Destination)
        {
        ...
        public string GetAllCreditMemoSAP(string SoldToParty, string formattedStartDate, string formattedEndDate)
        {
            ...
            var request1 = new HttpRequestMessage(HttpMethod.Get, "sap/opu/odata/sap/API_CREDIT_MEMO_REQUEST_SRV/A_CreditMemoRequest?$filter=CreditMemoRequestDate ge datetime'" + S_isoFormattedDate + "' and CreditMemoRequestDate le datetime'" + E_isoFormattedDate + "' and SoldToParty eq '" + SoldToParty + "'");
            string resString;
            ...
            try
            {
                ...
                HttpWebRequest req = (HttpWebRequest)HttpWebRequest.Create(SAPAPIURL + "sap/opu/odata/sap/API_CREDIT_MEMO_REQUEST_SRV/A_CreditMemoRequest?$filter=CreditMemoRequestDate ge datetime'" + S_isoFormattedDate + "' and CreditMemoRequestDate le datetime'" + E_isoFormattedDate + "' and SoldToParty eq '" + SoldToParty + "'");
                ...
                using (var client1 = new HttpClient(handler))
                {
                    ...
                    List<dynamic> CreditmemoHeader = new List<dynamic>();
                    ...
                    if (jsonResponse.ContainsKey("d"))
                    {
                        ...
                        if (res_Cnt > 0)
                        {
                            for (int i = 0; i < res_Cnt; i++)
                            {
                                ...
                                if (res_obj[i]["CreditMemoRequestDate"] != null && !string.IsNullOrEmpty(res_obj[i]["CreditMemoRequestDate"].ToString()))
                                {
                                    //creationDate = DateTime.Parse(res_obj[i]["DebitMemoRequestDate"]);
                                    //formattedDate = creationDate.ToString("yyyy-MM-dd");

                                    creationDate = DateTime.Parse(res_obj[i]["CreditMemoRequestDate"].ToString());
                                ...
                                {
                                    CreditMemoRequest = res_obj[i]["CreditMemoRequest"],
                                    CreditMemoRequestType = res_obj[i]["CreditMemoRequestType"],
                                    SoldToParty = res_obj[i]["SoldToParty"],
                                    PurchaseOrderByCustomer = res_obj[i]["PurchaseOrderByCustomer"],
                                    CreditMemoRequestDate = formattedDate,
                                ...
                                CreditmemoHeader.Add(jsonObject);
                            }
                        }
                    ...
                    return JsonConvert.SerializeObject(CreditmemoHeader);
        ...
        public string GetOneCreditMemo(string CreditMemoRequest)
        {

            //var resString;

            var request1 = new HttpRequestMessage(HttpMethod.Get, "sap/opu/odata/sap/API_CREDIT_MEMO_REQUEST_SRV/A_CreditMemoRequest('" + CreditMemoRequest + "')/to_Item");
            //var resString;
            ...
            try
            {
                ...
                HttpWebRequest req = (HttpWebRequest)HttpWebRequest.Create(SAPAPIURL + "sap/opu/odata/sap/API_CREDIT_MEMO_REQUEST_SRV/A_CreditMemoRequest('" + CreditMemoRequest + "')/to_Item");
                HttpWebResponse resp;
                    ...
                    if (jsonResponse.ContainsKey("d"))
                    {
                        ...
                        if (res_Cnt > 0)
                        {
                            for (int i = 0; i < res_Cnt; i++)
                            {
                                ...
                                dynamic jsonObject = new
                                {
                                    CreditMemoRequestItem = res_obj[i]["CreditMemoRequestItem"],
                                    CreditMemoRequestItemText = res_obj[i]["CreditMemoRequestItemText"],
