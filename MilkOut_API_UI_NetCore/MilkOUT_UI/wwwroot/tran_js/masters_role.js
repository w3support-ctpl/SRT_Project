...
function SaveEntry() {
  ...
  if (IsValid == 0) {
    ...
    $.ajax({
      ...
      error: function () {
        ShowEntryError("Error : Role details not saved");
      },
    });
...
function SaveDeleteEntry() {
  ...
  $.ajax({
    ...
    error: function () {
      ShowEntryError("Error : Role details not deleted");
    },
  });
}

function ShowRoleBaseReportEntry() {
  $("#modelEntry")
    .modal({
      backdrop: "static",
    })
    .modal("show");
  $("#ddlEntryRole").select2();
  GetMaster("ddlEntryRole", "Select Role", "GetUserRole", "", "");
  $("#ddlEntryRole").val("");
  ClearDataTable("tableRoleMenuReport");
}

function SaveRoleBaseReportEntry() {
  $("#modelEntry").modal("hide");
}

function GetRoleBaseReportEntry() {
  ClearDataTable("tableRoleMenuReport");
  var Role_Id = $("#ddlEntryRole").val();

  // Taking values from DB
  var APIEndPoint = "GetRole";
  var Method_Name = "Get_Report";
  var url = "/Masters/Role";
  var reqdata = {
    method_name: Method_Name,
    role_id: Role_Id,
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      $.each(res, function (data, value) {
        TableHTML += "<tr>";
        TableHTML += "<td style='width: 20px;'>" + (data + 1) + "</td>";
        TableHTML += "<td hidden>" + value.reporttype_id + "</td>";
        TableHTML += "<td>" + value.reporttype_name + "</td>";
      ...
      ClearDataTable("tableRoleMenuReport");
      $("#tableEntryReport").html(TableHTML);
      //SetDataTable("tableRoleMenuReport", [7], "Role Menu");
    },
    error: function (result) {
      ShowEntryError(
        "Error in fetching details from server.",
...
function SaveRoleBaseReportEntry() {
  ...
  if (Role_Id == "") {
    IsValid = 0;
    $("#ddlEntryRole").addClass("is-invalid state-invalid");
  }
  if (IsValid == 0) {
    ShowEntryError("Invalid Input(s). Can't be saved.");
    return;
  } else {
    var Method_Name = "Create_Report";
    ...
    // Getting Table values and converting it to XML
    RoleMenu += "<Menu>";
    $("#tableRoleMenuReport tbody tr").each(function () {
      var Display_Flag = 0;
      // set values of flags as 1 if checked
      if ($(this).find("td:eq(3) input").prop("checked") == true) {
        Display_Flag = 1;
      ...
      RoleMenu +=
        ...
        "</ReportType_Id>";
      RoleMenu += "<Flag>" + Display_Flag + "</Flag>";
      RoleMenu += "</MenuItem>";
    });
    RoleMenu += "</Menu>";
    ...
    $.ajax({
      ...
      success: function (res) {
        var result = JSON.parse(res);
        if (result[0].result_id == 1) {
          ShowEntrySuccess("Role Base Report details saved successfully");
          GetRoleBaseReportEntry();
        } else {
          ShowEntryError("Error : " + result[0].result_description);
        }
      },
      error: function () {
        ShowEntryError("Error : Role Base Report details not saved");
      },
    });
